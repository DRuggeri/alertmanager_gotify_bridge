...
func (svr *bridge) handle_call(w http.ResponseWriter, r *http.Request) {
	...
	if string(b) != "" {
		...
		if *svr.debug { log.Printf("Detected %d alerts\n", len(notification.Alerts)) }
		...
		for _, alert := range notification.Alerts {
			...
			if (proceed) {
				if *svr.debug { log.Printf("  Required fields found. Dispatching to gotify...\n"); }
				...
				if *svr.debug { log.Printf("  Outbound: %s\n", string(msg)); }
				...
				if err != nil {
					...
					if *svr.debug { log.Printf("  Dispatched! Response was %s\n", body); }
